...
$sqlPlaylist = "SELECT id, title FROM playlists";
...
<?php while($row_playlist = $playlist->fetch_assoc()) { ?>
    <a href="../pages/playlist.php?id='<?php echo $row_playlist['id'] ; ?>'" class="overflow-hidden rounded-xl relative group">
        <div class="rounded-xl z-50 opacity-0 group-hover:opacity-100 transition duration-300 ease-in-out cursor-pointer absolute from-black/100 to-transparent bg-gradient-to-t inset-0 bottom-0 pt-30 text-white flex items-end">
                <div class="p-4 text-xl group-hover:opacity-100 group-hover:translate-y-0 translate-y-2 pb-6 transform transition duration-300 ease-in-out">
                    <span class="font-bold"><?php echo $row_playlist["title"]; ?></span>
                </div>
        </div>
        <img alt="" class="object-cover w-full aspect-square group-hover:scale-110 transition duration-300 ease-in-out" src="<?php echo $dir. $row_playlist["id"] . '.jpg?n=' .time(); ?>"/>
    </a>
<?php } ?>
